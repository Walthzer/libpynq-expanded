...
//
int tofInit(iic_index_t iic, uint8_t addr, int bLongRange)
{
  sensor.iic_index = iic;
  sensor.baseAddr = addr;
...
static unsigned short readReg16(uint8_t ucAddr)
{
  ...
  iic_read_register(sensor.iic_index, sensor.baseAddr, ucAddr, ucTemp, 2);
...
static uint8_t readReg(uint8_t ucAddr)
{
  ...
  iic_read_register(sensor.iic_index, sensor.baseAddr, ucAddr, &ucTemp, 1);

	return ucTemp;
} /* ReadReg() */

static void readMulti(uint8_t ucAddr, uint8_t *pBuf, int iCount)
{

  iic_read_register(sensor.iic_index, sensor.baseAddr, ucAddr, pBuf, iCount);

} /* readMulti() */

static void writeMulti(uint8_t ucAddr, uint8_t *pBuf, int iCount)
{

  iic_write_register(sensor.iic_index, sensor.baseAddr, ucAddr, pBuf, iCount);
...
static void writeReg16(uint8_t ucAddr, unsigned short usValue)
{
  ...
  iic_write_register(sensor.iic_index, sensor.baseAddr, ucAddr, pBuf, 2);
} /* writeReg16() */
//
...
static void writeReg(uint8_t ucAddr, uint8_t ucValue)
{
  ...
  iic_write_register(sensor.iic_index, sensor.baseAddr, ucAddr, &ucValue, 1);
...
static void writeRegList(uint8_t *ucList)
{
  uint8_t ucCount = *ucList++; // count is the first element in the list
	...
	while (ucCount)
	{
    iic_write_register(sensor.iic_index, sensor.baseAddr, ucList[0], &(ucList[1]), 1);
		ucList += 2;
		ucCount--;
	}
} /* writeRegList() */
...
int tofGetModel(uint8_t *model, uint8_t *revision)
{
uint8_t ucTemp[2];
int i;
  ...
  sensor.baseAddr = 0x29;
	...
	if (model)
	{
		i = iic_read_register(sensor.iic_index, sensor.baseAddr, REG_IDENTIFICATION_MODEL_ID, ucTemp, 1);
		if (i == 0) //0 on succes aka no error
			*model = ucTemp[0];
	}
	if (revision)
	{
      i = iic_read_register(sensor.iic_index, sensor.baseAddr, REG_IDENTIFICATION_REVISION_ID, ucTemp, 1);
		if (i == 0)
			*revision = ucTemp[0];
	}
	return 1;
